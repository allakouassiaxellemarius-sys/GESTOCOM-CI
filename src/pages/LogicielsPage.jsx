...
import { Monitor, Tag, Key, HardDrive, FolderOpen, Search, X, CheckCircle, Lock } from 'lucide-react'
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Consultation en lecture seule — les modifications sont gérées par la plateforme</p>
        </div>
      </div>
      ...
      {tab === 'logiciels' && (
        logiciels.length === 0 ? (
          <div className="bg-white dark:bg-dark-800 rounded-2xl border border-gray-200 dark:border-dark-700 p-12 text-center">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Aucun logiciel n'est enregistré pour le moment.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {logiciels.map(l => (
              <div key={l.id} className="relative">
                <LogicielCard logiciel={l} versions={allVersions} licences={allLicences} />
              </div>
            ))}
          </div>
        )
