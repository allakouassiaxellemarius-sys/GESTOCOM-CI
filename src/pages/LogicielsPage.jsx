...
import { Monitor, Tag, Key, HardDrive, FolderOpen, Search, Plus, X, Trash2, Edit, Copy, Ban, CheckCircle, AlertTriangle, Lock } from 'lucide-react'
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            {isAdmin ? 'Gérez vos produits logiciels, versions, licences et déploiements' : 'Consultation — modification réservée à l\'administrateur'}
          </p>
        </div>
        {isAdmin && (
      ...
      {tab === 'logiciels' && (
        logiciels.length === 0 ? (
          <div className="bg-white dark:bg-dark-800 rounded-2xl border border-gray-200 dark:border-dark-700 p-12 text-center">
            ...
            {isAdmin && <button onClick={() => { setEditLogiciel(null); setShowForm(true) }} className="inline-flex items-center gap-2 px-4 py-2 bg-brand-600 hover:bg-brand-700 text-white rounded-lg text-sm font-semibold"><Plus className="w-4 h-4" /> Ajouter</button>}
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {logiciels.map(l => (
              <div key={l.id} className="relative group">
                <LogicielCard logiciel={l} onClick={isAdmin ? openEditLogiciel : undefined} versions={allVersions} licences={allLicences} />
                {isAdmin && (
        ...
        <div className="space-y-3">
          <div className="bg-white dark:bg-dark-800 rounded-2xl border border-gray-200 dark:border-dark-700 p-4">
            ...
            {isAdmin ? (
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500 flex items-center gap-1"><Lock className="w-3 h-3" /> Réservé à l'administrateur</p>
            )}
          </div>
          <div className="bg-white dark:bg-dark-800 rounded-2xl border border-gray-200 dark:border-dark-700 overflow-hidden">
            <div className="overflow-x-auto">
